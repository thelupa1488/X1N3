...
void CRadar::DrawMapImage(CBaseEntity* plocal)
{
	...
	if (CurrentMap.Name != XorStr("Null"))
	{
		...
		if (RadarMapImage == nullptr)
			FastCall::G().t_D3DXCreateTextureFromFileEx(GP_Render->m_pDevice, string(Utils::GetModuleBaseDir(0) + XorStr("\\csgo\\resource\\overviews\\") + CurrentMap.Name + XorStr("_radar.dds")).c_str(), 1223, 1223,
