...
static auto make_glove(int entry, int serial) -> CBaseAttributableItem*
{
	...
	assert(glove);
	{
		CSX::Memory::FindPattern(XorStr("client_panorama.dll"), XorStr("\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10"), XorStr("xxxxx????xxx"), 4);

		static auto set_abs_origin_addr = CSX::Memory::FindPattern(XorStr("client_panorama.dll"), XorStr("\x55\x8B\xEC\x83\xE4\xF8\x51\x53\x56\x57\x8B\xF1"), XorStr("xxxxxxxxxxxx"), 0);
