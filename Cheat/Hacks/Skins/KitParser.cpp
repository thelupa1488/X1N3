...
void Hook_SetViewModelSequence(const CRecvProxyData* pDataConst, void* pStruct, void* pOut)
{
	...
	if (pViewModel)
	{
		...
		if (pOwner && pOwner->EntIndex() == I::Engine()->GetLocalPlayer())
		{
			...
			//else if (!strcmp(szModel, XorStr("models/weapons/v_knife_css.mdl")))
			//{
			//	switch (m_nSequence)
			//	{
			//	case SEQUENCE_DEFAULT_LOOKAT01:
			...
			else if (!strcmp(szModel, XorStr("models/weapons/v_knife_widowmaker.mdl")))
			{
				...
				{
				case SEQUENCE_DEFAULT_LOOKAT01:
					m_nSequence = random_sequence(14, SEQUENCE_BUTTERFLY_LOOKAT03);
					break;
				}
			}
...
auto CSkins::initialize_kits() -> void
{
	...
	const auto sig_address = CSX::Memory::FindPattern(XorStr("client_panorama.dll"), XorStr("\xE8\x00\x00\x00\x00\xFF\x76\x0C\x8D\x48\x04\xE8"), XorStr("x????xxxxxxx"), 0);
	...
	{
		const auto sticker_sig = CSX::Memory::FindPattern(XorStr("client_panorama.dll"), XorStr("\x53\x8D\x48\x04\xE8\x00\x00\x00\x00\x8B\x4D\x10"), XorStr("xxxxx????xxx"), 4);
