...
namespace SDK
{
    ...
    KeyValues* KeyValues::FindKey(const char* keyName, bool create)
    {
       //static auto pFindKey = reinterpret_cast<KeyValues*(__thiscall*)(KeyValues*, const char*, bool)>(Utils::PatternScan(clientFactory, XorStr("E8 ? ? ? ? F7 45")) + 1);//need fix
        auto pFindKey = relativeToAbsolute<decltype(keyValuesFindKey)>(reinterpret_cast<int*>(Utils::PatternScan(clientFactory, "E8 ? ? ? ? F7 45") + 1));
    ...
    void KeyValues::SetString(const char* keyName, const char* value)
    {
        ...
        auto pSetSring = relativeToAbsolute<decltype(keyValuesSetString)>(reinterpret_cast<int*>(Utils::PatternScan(clientFactory, "E8 ? ? ? ? 89 77 38") + 1));
