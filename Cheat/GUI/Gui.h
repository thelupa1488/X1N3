...
#define DCheckBox(text, var)  X1Gui().CheckBox(XorStr(text), &var); \
/*ImGui::Checkbox(text, &var); \
*/

#define DComboBox(text, var, items) X1Gui().ComboBox(XorStr(text), &var, items); \
/* ImGui::Combo(text, &var, items, IM_ARRAYSIZE(items));\
*/

#define SliderInts(text, var,  min_, max_)  X1Gui().SliderInt(XorStr(text), &var, min_, max_); \
/*ImGui::SliderInt(text, &var, min, max);\
*/	

#define SliderIntsProc(text, var,  min_, max_) X1Gui().SliderIntProc(XorStr(text), &var, min_, max_); \
/* ImGui::SliderIntProc(text, &var, min, max);\
*/


#define SliderFloats(text, var,  min_, max_) X1Gui().SliderFloat(XorStr(text), &var, min_, max_); \
/*ImGui::SliderFloat(text, &var, min, max);\
*/

#define HotsKey(text, var) X1Gui().HotKey(XorStr(text), &var); \
/* ImGui::Hotkey(text, &var); \
*/

#define DColorEdit(text, var) X1Gui().ColorEdit2(XorStr(text), &var); /*ImGui::MyColorEdit5(text, var##._color, var##.brainbow, 1 << 7 )*/
